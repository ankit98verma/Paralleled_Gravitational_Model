...
__global__ void gpu_spherical_harmonics(float radius, int N_SPHERICAL, vertex* dev_R_vec, float* dev_coeff, float* U, int thread_index){
    ...
    float pseudo_coeff[N_spherical+1][N_spherical+2];
    for (int row =0; row<N_spherical+1; row++){
        for (int col= 0; col<N_spherical+2; col++){
            pseudo_coeff[row][col] = dev_coeff[row*(N_spherical+1) + col];
        }
    }
    ...
    for (int m=0; m<N_SPHERICAL+1; m++){
        for (int n = m; n<N_SPHERICAL+1; n++){
            ...
            if (m==0){
                ...
                S = N*pseudo_coeff[N_SPHERICAL-n][N_SPHERICAL-m+1];
            }
...
__global__
void kerne_gravitational(int g_vertices_length, float g_radius, float g_N_SPHERICAL, float* dev_coeff, vertex* dev_vertices, float* dev_potential){

    int thread_index = blockIdx.x * blockDim.x + threadIdx.x;

    while (thread_index < g_vertices_length) {

        gpu_spherical_harmonics(g_radius, g_N_SPHERICAL, dev_vertices[thread_index], dev_coeff, dev_potential, thread_index);
